...
io.on('connection', (socket) => {
    ...
    socket.on('match request', (name, difficulty, socketId) => {
        ...
        const pendingMatch = MatchingService
            .getPendingMatches(difficulty)
            .catch((err) => {
                console.log(err);
            })
        ...
        pendingMatch.then(data => {
            console.log(data)
            if (data.length === 0) {        // No match found, user will create a room as he initiates a match
                ...
                MatchingService
                    .createPendingMatch(name, difficulty, socketId)
                    .catch((err) => {
                        console.log(err);
                    })
            ...
            else if (data[1] != socketId) {         // check if another user has requested for a match
                // A match is found
                ...
                MatchingService
                    .deletePendingMatch(nameOfPendingMatch)
                    .catch((err) => {
                        console.log(err);
                    })
            } 
    ...
    function deleteUserPendingRequest(difficulty, socketId, meetingRoomId) {
        const userPendingMatch = MatchingService        
            .getPendingMatches(difficulty)
            .catch((err) => {
                console.log(err);
            })
        ...
        pendingMatchDeleted = userPendingMatch.then(data => {
            if (data.length > 0) {                      
                ...
                if (requestorID == socketId) {          
                    ...
                    MatchingService
                        .deletePendingMatch(requestorName)
                        .catch((err) => {
                            console.log(err);
                        })
                    ...
                    console.log(socket.rooms);
                    console.log("Timeout case: deleted pending request and left the room");
