...
app.get('/', (req, res) => {
    res.sendFile('C:\\Users\\Hong Ming\\Desktop\\GitHub\\CS3219 Project\\matching-service/temp_matching_service_file.html');
});
...
io.on('connection', (socket) => {
    socket.on('match request', (name, difficulty, socketId) => {
        ...
        pendingMatch.then(data => {
            console.log(data)
            if (data.length === 0) { // No match found, empty array
